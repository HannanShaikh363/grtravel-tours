...
class ProcessTourDestinationDataJob implements ShouldQueue
{
    ...
    public function handle(): bool
    {
        ...
        foreach ($this->chunkData as $column) {
            ...
            if (empty($column['location_id']) || empty($column['name'])) {
                ...
                if (filter_var($image, FILTER_VALIDATE_URL)) {
                    try {
                        ...
                        if ($response->successful()) {
                            ...
                            $filename = 'images/tours/tickets/' . uniqid() . '.' . pathinfo(parse_url($image, PHP_URL_PATH), PATHINFO_EXTENSION);
            ...
            try {
                ...
                $destinationData = [
                    ...
                    'ticket_title' => $column['ticket_title'] ?? '',
                ...
                Log::info("Processed record for Tour: {$column['name']} in location: {$city->name} Currency {$column['currency']}");
