...
class ProcessGentingRatesDataJob implements ShouldQueue
{
    ...
    public function handle(): bool
    {
        ...
        foreach ($this->chunkData as $column) {
            ...
            if (empty($column['hotel_name']) || empty($column['package'])) {
            ...
            $package = trim(strtolower($column['package']));
            $entitlements = !empty($column['entitlements']) ? explode('||', $column['entitlements']) : [];

            $hotel = GentingHotel::whereRaw('LOWER(TRIM(hotel_name)) = ?', [$hotel_name])->first();
            $package = GentingPackage::whereRaw('LOWER(TRIM(package)) = ?', [$package])->first();
            ...
            if (!$package) {
                Log::info('Invalid Package: ' . $package);
                return false;
            }
                ...
                if (filter_var($image, FILTER_VALIDATE_URL)) {
                    try {
                        ...
                        if ($response->successful()) {
                            ...
                            $filename = 'images/genting/rooms/' . uniqid() . '.' . pathinfo(parse_url($image, PHP_URL_PATH), PATHINFO_EXTENSION);
            ...
            try {
                ...
                $gentingData = [
                    'genting_hotel_id' => $hotel->id,
                    'genting_package_id' => $package->id,
                    'room_type' =>  $column['room_type'],
                    'images' => json_encode($imagePaths),
                    'price' => $column['price'],
                    'currency' => $column['currency'],
                    'entitlements' => json_encode($entitlements),
                    'bed_count' => $column['bed_count'],
                ...
                $gentingHotel = GentingRate::updateOrCreate(
                    [
                        'genting_hotel_id' => $hotel->id,
                        'genting_package_id' => $package->id,
                        'room_type' => $column['room_type'],
